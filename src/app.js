...
cron.schedule("1-59 * * * *", async () => {
  ...
  try {
    ...
    const averageTransactionPrice = salesRevenue > 0 && (orders && orders.length > 0) ? (salesRevenue / orders.length) : 0;
      ...
      for (const product of products) {
        ...
        for (const order of orders) {
          for (const productOfOrder of order.products) {
            if (product._id === productOfOrder.productId._id) {
    ...
    topFiveProductsSold = topFiveProductsSold?.sort((a, b) => a?.totalPrice - b?.totalPrice).slice(0, 4) || [],
